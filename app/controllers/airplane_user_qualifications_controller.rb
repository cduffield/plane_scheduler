class AirplaneUserQualificationsController < ApplicationController
  ...
  def create
    qualification = @airplane.airplane_user_qualifications.find_or_initialize_by(user_id: qualification_params[:user_id])
    qualification.assign_attributes(qualification_params.merge(approved_by: current_user))
